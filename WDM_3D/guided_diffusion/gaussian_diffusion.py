...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        # print('p mean variance x, input_masked', x.shape, input_masked.shape)

        assert t.shape == (B,)

        # print('x', x.shape)
        # print('input masked', input_masked.shape)
        ...
        def process_xstart(x):
            ...
            if clip_denoised:
                ...
                x = th.cat([LLL / 3.0, LLH, LHL, LHH, HLL, HLH, HHL, HHH], dim=1)
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            #  print('pred xstart', pred_xstart.shape, 'x_t', x.shape)
    ...
    def training_losses(
        ...
        # print('training losses x start, t', x_start.shape, t)
        y_masked = x_start[:, 0, :, :, :]
        y_mask = x_start[:, 1, :, :, :]
        y_gt = x_start[:, 2, :, :, :]
        ...
        y_masked = y_masked.unsqueeze(1) #y_masked = y_masked.unsqueeze(0).unsqueeze(1)
        y_mask = y_mask.unsqueeze(1) #y_mask = y_mask.unsqueeze(0).unsqueeze(1)
        y_gt = y_gt.unsqueeze(1) #y_gt = y_gt.unsqueeze(0).unsqueeze(1)

        # Wavelet transform the input image
        LLL, LLH, LHL, LHH, HLL, HLH, HHL, HHH = dwt(y_masked)
        x_masked_dwt = th.cat([LLL / 3.0, LLH, LHL, LHH, HLL, HLH, HHL, HHH], dim=1)
        # print('x masked dwt', x_masked_dwt.shape)

        # Wavelet transform the mask
        LLL, LLH, LHL, LHH, HLL, HLH, HHL, HHH = dwt(y_mask)
        x_mask_dwt = th.cat([LLL / 3.0, LLH, LHL, LHH, HLL, HLH, HHL, HHH], dim=1)
        # print('x masked dwt', x_masked_dwt.shape)

        # Wavelet transform the ground truth image
        LLL, LLH, LHL, LHH, HLL, HLH, HHL, HHH = dwt(y_gt)
        x_gt_dwt = th.cat([LLL / 3.0, LLH, LHL, LHH, HLL, HLH, HHL, HHH], dim=1)
        # print('x gt dwt', x_gt_dwt.shape)
        #pdb.set_trace()
        ...
        model_output_idwt = idwt(
            model_output[:, 0, :, :, :].view(B, 1, H, W, D) * 3.0,
        ...
        #   print('model output idwt', model_output_idwt.shape)

        terms = {"mse_wav": th.mean(mean_flat((x_gt_dwt - model_output) ** 2), dim=0)}
