...
class TrainLoop:
    ...
    def run_loop(self):
        ...
        while (
            ...
            #print(f"cond, cond shape, cond type {cond, cond.shape, cond.type}")
            ...
            #cond = {}
            cond = batch[:, 2, :, :, :] #cond = batch[0, 2, :, :, :]
            #batch = batch[0, :2, :, :, :]

            #print(f'batch cat size{batch.size()}, cond size{cond.size()}')
            #pdb.set_trace()
            t_fwd = time.time()
            t_load = t_fwd - t
            ...
            if self.summary_writer is not None:
                ...
                self.summary_writer.add_scalar(
                    "loss/MSE", lossmse.item(), global_step=self.step + self.resume_step
                )

            if self.step % 200 == 0:
                image_size = sample_idwt.size()[4]
                midplane = sample_idwt[0, 0, :, :, image_size // 2]
                self.summary_writer.add_image(
                    "sample/x_0",
                    midplane.unsqueeze(0),
                    global_step=self.step + self.resume_step,
                )

                image_size = sample.size()[4]
                for ch in range(8):
                    midplane = sample[0, ch, :, :, image_size // 2]
                    self.summary_writer.add_image(
                        "sample/{}".format(names[ch]),
                        midplane.unsqueeze(0),
                        global_step=self.step + self.resume_step,
                    )


                image_size = batch.size()[4]
                midplane = batch[0, 0, :, :, image_size // 2]
                self.summary_writer.add_image(
                    "sample/masked image",
                    midplane.unsqueeze(0),
                    global_step=self.step + self.resume_step,
                ...
                image_size = batch.size()[4]
                midplane = batch[0, 1, :, :, image_size // 2]
                self.summary_writer.add_image(
                    "sample/mask",
                    midplane.unsqueeze(0),
                    global_step=self.step + self.resume_step,
    ...
    def run_step(self, batch, cond, label=None, info=dict()):
        #print('size of batch', batch.size(), 'cond size', cond.size())
        #pdb.set_trace()
    ...
    def forward_backward(self, batch, cond, label=None):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            self.summary_writer.add_scalar(
                "loss/mse_wav_lll",
                losses["mse_wav"][0].item(),
                global_step=self.step + self.resume_step,
            )
            self.summary_writer.add_scalar(
                "loss/mse_wav_llh",
                losses["mse_wav"][1].item(),
                global_step=self.step + self.resume_step,
            )
            self.summary_writer.add_scalar(
                "loss/mse_wav_lhl",
                losses["mse_wav"][2].item(),
                global_step=self.step + self.resume_step,
            )
            self.summary_writer.add_scalar(
                "loss/mse_wav_lhh",
                losses["mse_wav"][3].item(),
                global_step=self.step + self.resume_step,
            )
            self.summary_writer.add_scalar(
                "loss/mse_wav_hll",
                losses["mse_wav"][4].item(),
                global_step=self.step + self.resume_step,
            )
            self.summary_writer.add_scalar(
                "loss/mse_wav_hlh",
                losses["mse_wav"][5].item(),
                global_step=self.step + self.resume_step,
            )
            self.summary_writer.add_scalar(
                "loss/mse_wav_hhl",
                losses["mse_wav"][6].item(),
                global_step=self.step + self.resume_step,
            )
            self.summary_writer.add_scalar(
                "loss/mse_wav_hhh",
                losses["mse_wav"][7].item(),
                global_step=self.step + self.resume_step,
            )

            weights = th.ones(
                len(losses["mse_wav"])
            ).cuda()  # Equally weight all wavelet channel losses

            #factors = torch.Tensor([1, 1, 1, 2, 1, 2, 2, 2]).cuda()
            #weights = weights * factors

            # Step 1: Compute the equally weighted MSE loss over all wavelet components
            mse_loss = (losses["mse_wav"] * weights).mean()

            # Step 2: Compute the high-frequency loss (HF components are indices 3, 5, 6, 7)
            high_freq_indices = [3, 5, 6, 7]
            high_freq_loss = torch.sum(torch.abs(losses["mse_wav"][high_freq_indices]))

            # Step 3: Combine with gamma factor
            gamma = 1  # Scaling factor for high-frequency loss
            loss = mse_loss + gamma * high_freq_loss
